	...
	return (
		<>
			...
			<Divider>
				...
				<Paragraph>
					Both the bot and the database require a few credentials to run. Starting with the database, you will to create
					a file named &quot;<i>.env.database.production.local</i>&quot; and then create some login credentials with the
					following template (put the login details in the quotation marks!):
				</Paragraph>
			...
			<Divider>
				...
				<CodeBlock clipboardText="docker compose --env-file ./.env.database.production.local -f compose.yaml up -d">
					<span>
						<span className="text-green-500">docker </span>
						<span>compose --env-file ./.env.database.production.local -f compose.yaml up -d</span>
					</span>
				</CodeBlock>
