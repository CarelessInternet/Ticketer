...
const command: Command = {
	category: 'Ticketing',
	data: new SlashCommandBuilder().setName('archive').setDescription('Archives the support ticket'),
	execute: async ({ interaction }) => {
		try {
			if (!interaction.channel!.isThread()) {
				return interaction.reply({
					content: 'You can only use this command in a support ticket which uses threads',
					ephemeral: true
				});
			}
