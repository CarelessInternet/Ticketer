...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!interaction.guild.me.permissions.has('KICK_MEMBERS')) return interaction.reply({content: 'I need the kick members permission to run this command', ephemeral: true}).catch(console.error);
    ...
    collector.on('end', (collected, reason) => {
      switch (reason) {
        case 'time': {
          embed.setDescription(`The kick on <@${member.user.id}> has been aborted due to no response`);
