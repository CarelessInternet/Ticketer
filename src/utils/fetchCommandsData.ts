 ...
 */
export const fetchCommandsData = async (): Promise<RESTPatchAPIApplicationCommandJSONBody[][]> => {
	...
	const globalCommands = await files.reduce(
		async (acc: Promise<RESTPatchAPIApplicationCommandJSONBody[]>, file: string) => {
